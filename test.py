...
def mainfunction(model_s, model_t, use_gpu, sourcepath_spatial, sourcepath_temporal):
    ...
    for picfile in filedir:
        ...
        print('judge:', judge, 'pre', pre, 'correct', correct, 'TP,FP,FN,TN', TP, FP, FN, TN, pre_spatical,
              pre_temporal, 0.01 * pre_spatical + 0.99 * pre_temporal)
    precision = TP / (TP + FP)
    recall = TP / (TP + FN)
...
def predict(modelpath_spatial, model_spaticial, sourcepath_spatial, modelpath_temporal, model_temporal,
            sourcepath_temporal):
    ...
    precision, recall, F1, accuracy, specificity = mainfunction(model_s, model_t, use_gpu, sourcepath_spatial,
                                                                sourcepath_temporal)
    return precision, recall, F1, accuracy, specificity
...
sourcepath_temporal = './dataset/optical_flow/Testset/'  # 光流输入数据

modelpath_spatial = './weights/spatial_weights/'  # 空间流模型位置
model_spaticial = capture_modelname(modelpath_spatial, sourcepath_spatial)
modelpath_temporal = './weights/temporal_weights/'  # 光流模型位置
...
for index in range(len(model_spaticial)):
    ...
    precision, recall, F1, accuracy, specificity = predict(modelpath_spatial, model_spaticial[index], \
                                                           sourcepath_spatial, modelpath_temporal,
                                                           model_temporal[index], sourcepath_temporal)
    print('**********')
    ...
    print('model:', model_spaticial[index], ':precision:', precision, ':recall:', recall, ':F1:', F1, ':accuracy:',
          accuracy, ':specificity:', specificity, file=mylog)
